import java.io.*;
import java.util.*;
public class gss2 implements Runnable 
{
	...
	public static void main(String[] args) throws Exception {	 new gss2().run();	 }
	...
	class Node
	{
		int pref,suff,sum,max,sd; HashMap<Integer,Integer> f;
		void assignLeaf(int value) 
		{
			pref=suff=max=sum=value; f=new HashMap();
			f.put(value, 1);
		}
		void merge(Node left, Node right) 
		{
			f=new HashMap(left.f);
			for(Map.Entry<Integer, Integer> e:right.f.entrySet())
			{
				int v=f.getOrDefault(e.getKey(), 0);
				f.put(e.getKey(), v+e.getValue());
			}
			for(Map.Entry<Integer, Integer> e: f.entrySet()) sd+=e.getKey()*(e.getValue()-1);
			sum = left.sum + right.sum -left.sd - right.sd;
