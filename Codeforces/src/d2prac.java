...
public class d2prac implements Runnable 
{
	...
	public void solve() 
	{
		int i; int n=in.ni(); long k=in.nl(), x=in.nl(); Long a[]=new Long[n];
		for(i=0;i<n;i++) a[i]=in.nl();
		Arrays.sort(a); 
		ArrayList<Long> gaps=new ArrayList();
		for(i=1;i<n;i++)
		{
			if(a[i]-a[i-1]>x) gaps.add(a[i]-a[i-1]);
		}
		long ans=gaps.size()+1;
		Collections.sort(gaps);
		for(i=0;i<gaps.size();i++)
		{
			long cgap=gaps.get(i);
			long need=cgap/x;
			if(cgap%x==0) need--;
			if(need>k) break;
			else 
